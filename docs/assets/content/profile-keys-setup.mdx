...
Set up your keys `bondId` and `userKey` for the `wns` service this is located in your profile directory: 
```
~/.wire/profile/<PROFILE>.yml
```

Here are the demo `bondId` and `userKey` credentials you could use:
